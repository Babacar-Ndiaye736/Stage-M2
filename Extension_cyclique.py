...
Source pour l'énumération des polynomes : projet https://gitlab.inria.fr/tnfs-alpha/alpha
...
tests supplémentaires : extension galoisienne, extension cyclique.
"""
import sys
...
from Extension_galoisienne import extension_galoisienne_poly

QQx = QQ['x']; (x,) = QQx._first_ngens(1)


def extension_cyclique_poly(hc, bound_p) :
    """ cette fonction teste si une extension est cyclique (condition plus forte que galoisienne)
    :param           hc: polynomial
    :param           bound_p: upper bound of prime number
    voir aussi la fonction extension_galoisienne dans Extension_galoisienne.py
    """

    L = prime_range(bound_p) #ici on definit une borne sup à choisir

    #extension_cyclique1 = False  # On suppose vrai au début
    ...
    for p in L:
        ...
        if poly.is_irreducible():
            if extension_galoisienne_poly(hc, bound_p):
                return True
    return False

def get_list_irr_poly_galois_cyclique(deg, max_coeff=1, bound_p=1000, monic=True, output_file="", onthefly=False, start_counter=None, stop_counter=None, verbose=False, only_attained_max_coeff=False):
    ...
    while counter < max_counter:
        ...
        if h.is_irreducible() :
            Test_poly=extension_cyclique_poly(h, bound_p)
...
if __name__ == "__main__":
    ...
    get_list_irr_poly_galois_cyclique(deg=d, max_coeff=1, monic=True, output_file="test_list_polys_extension_cyclique_deg"+str(d)+".py", onthefly=True, start_counter=None, stop_counter=None, verbose=False, only_attained_max_coeff=False)
